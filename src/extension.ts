...
export function activate(context: vscode.ExtensionContext) {
  ...
  console.log("Congratulations, your extension 'angular-nestjs' is now active!");
  if (workspaceFolders) {
    ...
    if (!fs.existsSync(angularConfigPath)) {
      return updateIconPath("angular");
    } else if (!fs.existsSync(nestConfigPath1)) {
      return updateIconPath("nestjs");
    }
  }
}
